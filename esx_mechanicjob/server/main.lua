...
RegisterServerEvent('esx_mechanicjob:betaalanwbpas')
AddEventHandler('esx_mechanicjob:betaalanwbpas', function()
    local _source    = source
    local xPlayer = ESX.GetPlayerFromId(_source)    
    --Speler online
    if xPlayer then
        xPlayer.removeAccountMoney('bank', 5000)
        TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = 'Je hebt betaald voor je ANWB lidmaatschap.'})
        TriggerEvent('esx_addonaccount:getSharedAccount', 'society_mechanic', function(account)
